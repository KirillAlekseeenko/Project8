...
public class CoinParty : MonoBehaviour {
	...
	void Update () {
		...
		if(!firstFrame) {
			...
			for(int i = 0; i < glowID; i++) {
				glowParticles[i].position = parkPos;
				glowParticles[i].remainingLifetime = longLifetime;
			...
			for(int i = 0; i < maxCoins; i++) {
				if(coins[i].lifetime < longLifetime-1) {
					...
					if(coins[i].lifetime < 0 || collected) {
						...
						coinParticles[i].position = parkPos;
						coins[i].lifetime = longLifetime;
						coinParticles[i].remainingLifetime = longLifetime;
					...
					// apply coin rotation
					coinParticles[i].remainingLifetime = arbitraryLifetime + arbitraryLifetime*coins[i].rotation*recip360;
					...
					if(max > glowThreshold) {
						...
						glowParticles[glowID].remainingLifetime = arbitraryLifetime + arbitraryLifetime*coins[i].rotation*recip360;
	...
	void Emit () {
		...
		coinParticles[coinID].remainingLifetime = minLifetime*2;
